  ...
  return (
    ...
    >
      <div className="flex items-start gap-4">
        <div className="w-12 h-12 rounded-full bg-gradient-to-r from-blue-500/20 to-purple-500/20 flex items-center justify-center text-blue-300 text-xl flex-shrink-0 border border-blue-400/30">
          {icon}
        </div>
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {skills && skills.map((skill, idx) => (
              <span
                key={idx}
                className="px-2 py-1 text-xs rounded-full bg-gradient-to-r from-blue-500/20 to-purple-500/20 text-gray-300 border border-blue-400/30"
              >
                {skill}
              </span>
            ))}
...
const StatCard = ({ icon, value, label, delay }) => {
  return (
    ...
    >
      <div className="w-16 h-16 rounded-full bg-gradient-to-r from-blue-500/20 to-purple-500/20 flex items-center justify-center text-blue-300 text-2xl mx-auto mb-4 border border-blue-400/30">
        {icon}
      </div>
      <h3 className="text-heading-2 text-white">{value}</h3>
          <p className="text-body text-gray-300">{label}</p>
    </motion.div>
  );
};
    ...
    <section id="experience" className="section-spacing bg-black relative overflow-hidden" ref={sectionRef}>
      ...
      <div className="container-spacing">
        ...
        <motion.div
          ...
          className="mt-20"
        >
          <h3 className="text-2xl font-bold bg-gradient-to-r from-blue-300 to-purple-400 bg-clip-text text-transparent text-center mb-10 relative inline-block">
            Achievements & Stats
